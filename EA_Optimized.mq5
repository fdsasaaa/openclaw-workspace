...
input double   质量评分阈值          = 1.0;    // 🔥 修改：画出所有箱体   // 🔥 优化：只交易高质量箱体   // 🔥 修改：从20改为35（与TV策略一致）
input int      ATR周期               = 14;     // ATR周期 (TradingView默认14)
input double   ATR止损倍数           = 1.2;    // 🔥 优化：收紧止损    // 🔥 修改：从2.0改为1.7（与TV策略一致）
...
int         移动止损步长          = 65;
// 移动止损步长（固定点数模式使用）
double      箱体高度触发倍数      = 0.50;  // 🔥 优化：更早锁定利润  // 🔥 修改：从0.65改为0.85（与TV策略一致）
// (原g_BoxHeightTriggerMultiplier) 箱体高度触发倍数
double      箱体高度步长倍数      = 0.32;
double      箱体高度回撤倍数      = 0.25;  // 🔥 优化：更紧跟踪  // 🔥 修改：从0.32改为0.45（与TV策略一致）
